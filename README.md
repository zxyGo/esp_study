...
唤醒后可以直接说“声音大一点”“声音小一点”“音量调到 50”“最大音量”“静音”或“取消静音”。音量范围为 0～100%，调大/调小默认每次变化 10%，指令由 ESP32 本地执行，不会发送给大模型；设置值保存在 NVS，重启后仍然有效。`100%` 保持 TTS 返回的原始 PCM 幅度，不会额外数字放大，因此可避免削波失真。默认音量和步长可通过 `SPEAKER_VOLUME_DEFAULT`、`SPEAKER_VOLUME_STEP` 修改。
