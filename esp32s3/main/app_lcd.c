...
void app_lcd_draw_stt_frame(void)
{
    if (lvgl_port_lock(0)) {
        ...
        create_label(&s_gps_lat_label, "LAT: waiting for GPS", 92, C_ORANGE);
        create_label(&s_gps_lon_label, "LON: waiting for GPS", 113, C_ORANGE);
                     ...
                     bool fix_valid)
{
    ...
    if (fix_valid) {
        ...
        snprintf(lat_text, sizeof(lat_text), "LAT: searching satellites");
        snprintf(lon_text, sizeof(lon_text), "LON: move outdoors");
    } else {
        snprintf(lat_text, sizeof(lat_text), "LAT: no NMEA data");
        snprintf(lon_text, sizeof(lon_text), "LON: check RX GPIO%d", GPS_RX);
    }
    snprintf(utc_text, sizeof(utc_text), "UTC: %s",
             (utc && utc[0]) ? utc : "--:--:--");

    label_set(s_gps_lat_label, lat_text, fix_valid ? C_GREEN : C_ORANGE);
    label_set(s_gps_lon_label, lon_text, fix_valid ? C_GREEN : C_ORANGE);
